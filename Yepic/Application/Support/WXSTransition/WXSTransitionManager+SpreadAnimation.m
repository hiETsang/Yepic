...
- (void)pointSpreadNextWithContext:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    if (self.startView) {
        rect = CGRectMake(self.startView.frame.origin.x, self.startView.frame.origin.y, self.startView.frame.size.width, self.startView.frame.size.height);
    }
...
//        rect = CGRectMake(tempCenter.x - 1, tempCenter.y - 1, 2, 2);
        rect = CGRectMake(self.startView.center.x, self.startView.center.y, 1, 1);
    }
