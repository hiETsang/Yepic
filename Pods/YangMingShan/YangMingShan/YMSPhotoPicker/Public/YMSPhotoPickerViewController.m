...
- (void)viewDidLoad
{
    ...
    if (![self authorizationStatusAuthorized]) {
        if ([self.delegate respondsToSelector:@selector(photoPickerViewControllerDidReceivePhotoAlbumAccessDenied:)]) {
            [self.delegate photoPickerViewControllerDidReceivePhotoAlbumAccessDenied:self];
        }
